 ...
 * </pre>
 */
class CompatibleRpcServiceExporter implements BeanFactoryPostProcessor {

	private static final Logger LOG = Logger.getLogger(CompatibleRpcServiceExporter.class.getName());
		...
		for (String beanName : beanFactory.getBeanDefinitionNames()) {
			...
			if (jsonRpcPath != null) {
				...
				if (serviceBeanNames.containsKey(pathValue)) {
					...
					LOG.warning(format("Duplicate JSON-RPC path specification: found %s on both [%s] and [%s].", pathValue, beanName, otherBeanName));
				}
	...
	private void registerServiceProxy(DefaultListableBeanFactory dlbf, String servicePath, String serviceBeanName) {
		BeanDefinitionBuilder builder = BeanDefinitionBuilder.rootBeanDefinition(CompatibleJsonServiceExporter.class).addPropertyReference("service", serviceBeanName);
		...
		for (Class<?> iface : getBeanInterfaces(serviceBeanDefinition, dlbf.getBeanClassLoader())) {
			if (iface.isAnnotationPresent(JsonRpcService.class)) {
				...
				LOG.fine(format("Registering interface '%s' for JSON-RPC bean [%s].", serviceInterface, serviceBeanName));
	...
	/**
	 * @param backwardsComaptible
	 *            the backwardsComaptible to set
	 */
	...
	/**
	 * @param allowLessParams
	 *            the allowLessParams to set
	 */
	...
	/**
	 * See
	 * {@link org.springframework.remoting.support.RemoteExporter#setRegisterTraceInterceptor(boolean)}
	 * 
	 * @param registerTraceInterceptor
